...
function test_plugin_setup_menu(){
    $menu_test_plugin = add_submenu_page( 'wpcf7', 'Test Plugin','Test Plugin', 'manage_options', 'test-plugin', 'test_init' );
    remove_submenu_page( 'wpcf7', 'test-plugin' );
    add_action('load-' . $menu_test_plugin, 'cf7d_form_action_callback');
}
...
function test_init()
{
    ...
    ?>
    <div class="wrap">
        <h2>WP_List_Table Class Example 2222</h2>
    ...
    echo '<div class="wrap"><h1>' . __('2222') . '</h1>';
    ...
    if (!empty($fid)) {
        ...
        <form action="" method="GET" id="cf7d-admin-action-frm 1234">
            ...
            <table class="wp-list-table widefat fixed striped posts cf7d-admin-table 5678">
                <thead>
                <tr>
                    <?php
...
function cf7d_custom_submenu_page_callback()
{
    ...
    echo '<form action="'.admin_url('admin.php').'" method="GET">';
    echo '<input type="hidden" name="page" value="cf7-data" />';
    echo '<select name="fid" onchange="this.form.submit();" id="cf7d-export_select">';
    echo '<option value="" '.(( count($list_table->items) == 0 ) ? 'selected="selected"' : '').' >'.__('Choose form').'</option>';
    $i = 0;
    foreach ($list_table->items as $k => $v) {
        if ($first_form_id == 0 && $i == 0) {
